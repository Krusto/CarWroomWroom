...
void ReadDataByIdentifierHandler::MainHandle(Message message) {
    ...
    if (message.length() != 3) {
        ...
        uint16_t did = (((uint16_t) message[1]) << 8) | ((uint16_t) message[2] & 0xFF);
...
int ReadDataByIdentifierHandler::HandleDID(uint16_t did, Message& message) {
    switch (did) {
        case RDID_VW_Logical_Software_Block_Counter_Of_Programming_Attempts:{
            for (size_t i = 0; i < 14; i++)
                AddByteToRespBuffer(0x00);
            break;
        } 
        case RDID_VW_Logical_Software_Block_Lock_Value:{
            for (size_t i = 0; i < 14; i++)
                AddByteToRespBuffer(0x00);
            break;
        }
        ...
        case RDID_VW_logical_block_downgrade_protection_versions:{
            uint8_t arr[] = {0x01,0x30,0x30,0x30,0x35,0x30,0x30,0x30,0x36,0x30,0x30,0x30,0x35,0x30,0x30,0x30,0x36,0x30,0x30,0x30,0x35,0x30,0x30,0x30,0x35};
            AddByteArrayToRespBuffer(arr, sizeof(arr)/sizeof(uint8_t));
            break;
        }
        case RDID_VWECUHardwareNumber:{
            for (size_t i = 0; i < 11; i++)
            {
                AddByteToRespBuffer('_');
            }
        ...
        case RDID_VWECUHardwareVersionNumber:{
            AddByteToRespBuffer('-');
            AddByteToRespBuffer('-');
            AddByteToRespBuffer('-');
        }
        ...
        case 0xFFFF:{
                    uint16_t address = ((uint16_t)message[3]<<8) | ((uint16_t)message[4] & 0xFF);
                    switch(address){
                        case RDID_SoftwareVersionNumber:
                        {
