...
void ReadMemByAddressHandler::MainHandle(CAN_Message message) {
    InitResponseBuffer();
    if (SessionData::GetSecurityAccessStatus() == false) {
        ...
        uint8_t addressLength = message[2] & 0x0F;
        uint8_t sizeLength = (message[2] & 0xF0) >> 4;

        uint32_t address = 0;
        for (uint8_t i = 1; i <= addressLength; ++i) {
            address |= (uint32_t) message[2 + i] << (8 * (addressLength - i));
        }
        uint32_t size = 0;
        for (uint8_t i = 1; i <= sizeLength; ++i) {
            size |= (uint32_t) message[2 + addressLength + i] << (8 * (sizeLength - i));
        }

        if ((address > MEM_SIZE) || (address + size > MEM_SIZE)) {
            SendNegativeResponse(NRC::RequestOutOfRange);
        } else if (message[0] != 2 + addressLength + sizeLength) {
            SendNegativeResponse(NRC::IncorrectMessageLengthOrInvalidFormat);
        } else {
